...
let eventLoop = (options, schedule, logger) => {
  ...
  return {
    origSchedule,
    nonAnnotatedHistory: history, // <- this is the one being tested by jest
    history: annotateHistory(history, schedule),
  };
};
...
let annotateHistory = (history, schedule) => {
...
let printData = ({ history, origSchedule }, options) => {
  let taskevents = _.join(
    _.map(origSchedule.tasks, (t) => {
      return [
  ...
  let s = `
  \\begin{itemize}
  \\item Dati scheduling: $\\bar{\\tau}$= ${origSchedule.class.latency}, $\\mu$=${origSchedule.class.mingran}, $\\omega$=${origSchedule.class.wgup}
  ${taskevents}
  \\end{itemize}`;
...
let drawHistory = ({ history, origSchedule }, options, logger) => {
  let hs = origSchedule.graphics.hspace;
  let vs = origSchedule.graphics.vspace;
  let hh = origSchedule.graphics.barheight;
  ...
  let diag = _.map(history, (x, i) => {
    if (x.tstart < origSchedule.runfor) {
  ...
  let grid = [
    `\\draw[xstep=${origSchedule.timer},gray!20,thin,shift={(0,-0.25)}] (0,0) grid (${origSchedule.runfor},${origSchedule.tasks.length});`,
    _.map(_.range(0, origSchedule.runfor / origSchedule.timer + 1), (i) =>
      printAtConf(
        i * origSchedule.timer,
        -0.7,
        `\\emph{${i * origSchedule.timer}}`,
        "text=gray"
      )
    ),
  ];
  logger.debug(origSchedule.tasks);

  let taskevents = _.map(origSchedule.tasks, (t) => {
    return [
      printAtConf(
        0,
        (-origSchedule.tasks.length + t.index) * 0.2 - 0.7,
  ...
  let taskexits = _.map(origSchedule.tasks, (t) => {
  ...
  let data = [
    printAtConf(
      -0.6,
      origSchedule.tasks.length,
      `Schedule data: $\\bar{\\tau}$= ${origSchedule.class.latency}, $\\mu$=${origSchedule.class.mingran}, $\\omega$=${origSchedule.class.wgup}`,
      "anchor=west"
    ),
  ];
...
let exportLatex = (options, sim, logger) => {
  return {
    complete: latexArtifact(
      drawHistory(sim, { blank: false }, logger),
      "rt diagram",
      "standalone",
      "pdflatex",
      "-r varwidth"
    ...
    blank: latexArtifact(
      drawHistory(sim, { blank: true }, logger),
      "rt diagram blank",
      "standalone",
      "pdflatex",
      "-r varwidth"
