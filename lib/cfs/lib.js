...
let drawHistory = (history, schedule, finalschedule, options) => {
  ...
  let grid = [
    `\\draw[xstep=${schedule.timer},gray!20,thin,shift={(0,-0.25)}] (0,0) grid (${schedule.runfor},${schedule.tasks.length});`,
  ...
  let data = [
    printAtConf(
      ...
      `Schedule data: $\\bar{\\tau}$= ${schedule.class.latency}, $\\mu$=${schedule.class.mingran}, $\\omega$=${schedule.class.wgup}`,
      "anchor=west"
    ),
  ];
